...
if($tab === 'edit'){
    ...
    if(isset($_POST['bulk_duplicate'])){
        ...
        if(isset($_POST['selected_machines']) && is_array($_POST['selected_machines'])){
            $dup_stmt = $conn->prepare(
                "INSERT INTO machines (machine_no, smibb_ip, mac, pos_x, pos_y, pos_z, rotation, note)
                 SELECT CONCAT(machine_no, ' (Kopya)'), smibb_ip, mac, pos_x + 20, pos_y + 20, pos_z, rotation, CONCAT(COALESCE(note,''), ' (Kopya)')
    ...
    if(isset($_POST['save'])){
        ...
        if(isset($_POST['id']) && !empty($_POST['id'])){
            $id = intval($_POST['id']);
            $stmt = $conn->prepare("UPDATE machines SET machine_no=?, smibb_ip=?, mac=?, pos_z=?, note=? WHERE id=?");
            $stmt->bind_param("sssisi", $machine_no, $smibb_ip, $mac, $pos_z, $note, $id);
            $stmt->execute();
            $stmt->close();
            $message = "Makine güncellendi";
        } else {
            $stmt = $conn->prepare("INSERT INTO machines (machine_no, smibb_ip, mac, pos_z, note) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssis", $machine_no, $smibb_ip, $mac, $pos_z, $note);
            $stmt->execute();
            $stmt->close();
...
<?php if($tab === 'edit'): ?>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th>ID</th><th>Makine No</th><th>Smibb IP</th><th>MAC Adresi</th>
            ...
            <tbody>
                <?php if($machines->num_rows > 0): ?>
                    ...
                    <tr data-id="<?php echo intval($row['id']); ?>" class="machine-row">
                        ...
                        <td>
                            <button class="action-btn edit-btn" onclick="editMachine(<?php echo intval($row['id']); ?>,'<?php echo htmlspecialchars($row['machine_no'],ENT_QUOTES); ?>','<?php echo htmlspecialchars($row['smibb_ip'],ENT_QUOTES); ?>','<?php echo htmlspecialchars($row['mac'],ENT_QUOTES); ?>',<?php echo intval($row['pos_z']); ?>,'<?php echo htmlspecialchars($row['note']??'',ENT_QUOTES); ?>')">Düzenle</button>
    ...
    <script>
        ...
        function editMachine(id,no,smibb_ip,mac,z,note){ document.getElementById('edit_id').value=id; document.getElementById('edit_machine_no').value=no; document.getElementById('edit_smibb_ip').value=smibb_ip; document.getElementById('edit_mac').value=mac; document.getElementById('edit_z').value=z; document.getElementById('edit_note').value=note; document.querySelector('.form-box').scrollIntoView({behavior:'smooth'}); }
        function clearForm(){ ['edit_id','edit_machine_no','edit_smibb_ip','edit_mac','edit_note'].forEach(id=>document.getElementById(id).value=''); document.getElementById('edit_z').value='0'; }
...
<?php elseif($tab === 'delete'): ?>
    ...
    <form method="get" action="machine_settings.php" style="display:flex;gap:10px;margin-bottom:20px;">
        ...
        <input type="text" name="search" placeholder="Makine no, Smibb IP veya MAC ile ara..."
